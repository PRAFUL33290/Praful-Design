/**
 * Atelier Nuance — Interactions
 * Navigation, reveal, before/after, lightbox, formulaires
 */
...
(function () {
  ...
  if (baSlider) {
    ...
    baSlider.addEventListener("keydown", (e) => {
      ...
      if (e.key === "ArrowLeft") {
        setPosition(
          baSlider.getBoundingClientRect().left +
            ((current - 3) / 100) * baSlider.offsetWidth
        );
      }
      if (e.key === "ArrowRight") {
        setPosition(
  ...
  /* ---------- Color simulator ---------- */
  const sim = document.querySelector("[data-color-sim]");
  if (sim) {
    const preview = sim.querySelector(".color-sim__preview-room");
    const labels = sim.querySelectorAll("[data-sim-label]");
    const code = sim.querySelector("[data-sim-code]");
    const swatches = sim.querySelectorAll(".color-sim__swatch");
    ...
    swatches.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (preview) {
          preview.style.setProperty("--sim-wall", color);
          preview.style.background = color;
        }
        labels.forEach((label) => {
          label.textContent = name || "";
        });
        if (code) code.textContent = (ref || "") + (color ? " · " + color : "");
      });
    });
  }
